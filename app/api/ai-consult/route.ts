...
const AGENTS = [
  {
    ...
    name: '統括AI',
    title: 'チーフディレクター',
    system: '你はAI会議の統括ディレクターです。相談内容の本質を整理し、タスクを分類して会議の議題を設定します。必ずJSON形式のみで返答してください。',
...
JSON形式で返してください：
{
  "taskType": "記事作成 | アプリ開発 | 事業アイデア | 調査 | 比較検討 | 学習計画 | その他",
  ...
  },
  {
    ...
    name: '推進戦略AI',
    title: '戦略アドバイザー',
    system: 'あなたは推進戦略を担うAIアドバイザーです。アイデアのメリット・成功条件・実現可能性を前向きに分析します。必ずJSON形式のみで返答してください。',
...
JSON形式で返してください：
{
  ...
  },
  {
    ...
    name: 'リスク分析AI',
    title: 'リスクアナリスト',
    system: 'あなたはリスク分析を担うAIアナリストです。潜在リスク・弱点・懸念事項を客観的に指摘します。必ずJSON形式のみで返答してください。',
...
JSON形式で返してください：
{
  ...
  },
  {
    ...
    name: '意思決定AI',
    title: 'デシジョンメイカー',
    system: 'あなたは意思決定を担うAIです。戦略面・リスク面の両方を踏まえ、現実的で実行可能な結論を導きます。必ずJSON形式のみで返答してください。',
...
JSON形式で返してください：
{
  ...
  },
  {
    ...
    name: '実行計画AI',
    title: 'プロジェクトマネージャー',
    system: 'あなたは実行計画を担うAIプロジェクトマネージャーです。具体的な行動計画・タスクリスト・プロンプトを作成します。必ずJSON形式のみで返答してください。',
...
JSON形式で返してください：
{
  ...
  "readyPrompt": "この相談に基づきAIに渡すプロンプト（詳細な指示文500文字程度）"
}`,
  },
]
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({
    content: {
      ...
      todoNow:    execution?.todoNow   ?? [],
      nextTasks:  execution?.nextTasks ?? [],
      readyPrompt: execution?.readyPrompt ?? '',
    },
  })
}
